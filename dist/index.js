...
import { convertImages } from "./utils/image-converter.js";
import { convertVideos } from "./utils/video-converter.js";
import { scanMedia, scanCode } from "./utils/scanner.js";
import { patchJS } from "./utils/patchers/js.js";
const program = new Command();
program
    .name("optimist")
    .description("CLI tool that optimizes images and videos for React/Next.js projects")
    .version("1.0.0")
    ...
    console.log(chalk
        .hex("#F1F1F1")
        .dim("CLI tool to optimize images and videos for React/Next.js projects\n"));
    ...
    console.log(chalk.blue(`Found ${images.length} image(s), ${videos.length} video(s), ${codeFiles.length} code file(s)\n`));
    ...
    if (!options.skipPatch && codeFiles.length > 0) {
        ...
        if (Object.keys(mapping).length > 0) {
            ...
            const { updatedFilesCount, parseFailureFiles } = await patchJS(codeFiles, mapping, targetDir, isDryRun);
            console.log(chalk.green(`✔ Updated references in ${updatedFilesCount} file(s)`));
            if (parseFailureFiles.length > 0) {
                console.log(chalk.yellow(`⚠ Could not parse ${parseFailureFiles.length} file(s):`));
